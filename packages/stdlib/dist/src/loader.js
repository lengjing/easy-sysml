...
function findStdlibPath() {
    ...
    if (envPath) {
        ...
        console.warn(`[stdlib] SYSML_STDLIB_PATH="${envPath}" does not contain Base.kerml — falling back to auto-detection`);
    }
 ...
 * ```
 */
export async function loadStdLib(services, options = {}) {
    ...
    const documentFactory = services.workspace.LangiumDocumentFactory;
    const allDocuments = [];
    let loadedCount = 0;
