...
import type { AstNode, PackageNode, MembershipNode, PartDefinitionNode, PartUsageNode, FeatureTypingNode } from '@easy-sysml/ast';
...
function makePartDefinition(name: string, specializations: Array<{ general: string }> = []): PartDefinitionNode {
  return {
    $type: 'PartDefinition',
    $id: makeId(),
    name,
...
function makePartUsage(name: string, typings: string[] = []): PartUsageNode {
  return {
    $type: 'PartUsage',
    $id: makeId(),
    name,
    typings: typings.map((t) => ({
      $type: 'FeatureTyping' as const,
      $id: makeId(),
      type: t,
    })),
    members: [],
...
function makeMembership(element: AstNode, memberName?: string): MembershipNode {
  return {
    $type: 'Membership',
    $id: makeId(),
    visibility: 'public',
...
function makePackage(name: string, members: MembershipNode[]): PackageNode {
  return {
    $type: 'Package',
    $id: makeId(),
    name,
...
function makeRootPackage(members: MembershipNode[]): PackageNode {
  return {
    $type: 'Package',
    $id: makeId(),
    members,
...
describe('SymbolTable', () => {
  it('should store and retrieve symbols', () => {
    ...
    const symbol = {
      ...
      node: { $type: 'PartDefinition', $id: '1' } as AstNode,
      scope,
    };
  ...
  it('should retrieve symbol by node', () => {
    const table = new SymbolTable();
    const scope = new Scope('root');
    const node = { $type: 'PartDefinition', $id: 'node-1' } as AstNode;
    const symbol = {
      name: 'Engine',
  ...
  it('should handle multiple symbols with the same name', () => {
    ...
    const sym2 = {
      ...
      kind: SymbolKind.DEFINITION,
      node: { $type: 'PartDefinition', $id: '2' } as AstNode,
      scope,
    };
  ...
  it('should clear all symbols', () => {
    ...
    table.addSymbol({
      ...
      node: { $type: 'PartUsage', $id: '2' } as AstNode,
      scope,
    });
    table.clear();
...
describe('TypeSystem', () => {
  ...
  it('should get all features including inherited', () => {
    ...
    const baseType = {
      ...
      supertypes: [] as import('../type-system.js').SysMLType[],
